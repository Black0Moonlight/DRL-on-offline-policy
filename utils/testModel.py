...
from algo.DDPG2 import DDPGAgent2
...
def testandtrainNet(net_num, state_dim, action_dim, net):
    if net == "DDPG":
    ...
    for i in range(200):
        ...
        for j in range(opt.max_steps_one_episode):
            ...
            s0 = s1
            if replay_buffer.__len__() > 16:
                loss = Agent.update(replay_buffer.sample(16))
            # time.sleep(0.1)
        score += is_success
        # time.sleep(0.3)
    print('/***************** NO.{} network {:.1f}% ******************/'.format(net_num, score/2))
...
def train_xyz_DDPG(state_dim, action_dim, batch_size, action_bound=1):
    print("/************************* train xyz **************************/")
    # 使用ReplayMemory，可以使用sample和一条在线策略的on sample，注意调整代码
...
def train_xyz_DDPG_(state_dim, action_dim, batch_size, action_bound=1):
    print("/************************* train xyz **************************/")

    action_bound = 0.3 + float(env.action_space.high[0])
    Agent = DDPGAgent2(state_dim, action_dim, action_bound=action_bound)
...
def train_joint_DDPG_(state_dim, action_dim, batch_size, action_bound=1):
    print("/************************* train joint ************************/")
    # 使用ReplayMemory1
    Agent = DDPGAgent(state_dim, action_dim, action_bound=action_bound)
    last_total_r = 0
...
if __name__ == '__main__':
    ...
    # testNet(1, 6, 3, "DDPG")
    # train_xyz_DDPG_(state_dim=6, action_dim=3, batch_size=16)
    # train_xyz_DDPG(state_dim=6, action_dim=3, batch_size=16)
    # train_xyz_SAC(state_dim=6, action_dim=3, batch_size=16)
    train_joint_DDPG(state_dim=13, action_dim=7, batch_size=16)
    # train_joint_DDPG_(state_dim=13, action_dim=7, batch_size=16)
